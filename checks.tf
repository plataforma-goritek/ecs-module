...
check "security_group_ingress_required" {
  assert {
    condition     = !var.create_security_group || length(var.allowed_security_group_ids) > 0 || length(var.allowed_cidr_blocks) > 0 || (var.create_load_balancer || var.deployment_mode == "production") || trimspace(coalesce(var.target_group_arn, "")) != ""
